...
class RateLimitExceeded(RuntimeError):
    ...
    있게 한다(2026-08-27, 사용자 요청: "실제 연결한 모델의 API"가 알려주는 실측 한도로 자동 조절 —
    제공자별로 미리 값을 추측/하드코딩하는 대신, 매 실패에서 그 제공자가 직접 알려주는 정확한 수치를
    그대로 쓴다).
    ...
    limit/requested 수치로 판단한다 — 제공자마다 상태 코드 관례가 다르다(`llm_backend.py`의
    `_parse_rate_limit_error()` docstring 참고: Groq는 413/429로 나누지만 OpenAI는 같은
    상황도 전부 429로 보낸다)."""
...
def _parse_rate_limit_error(code: int, body_text: str, headers) -> "RateLimitExceeded | None":
    ...
    절반씩 줄이게 `limit_tokens`/`requested_tokens`를 None으로 둔다 — `error.code`나 표준
    `x-ratelimit-*` 헤더 존재만으로도 "토큰 한도 초과"라는 사실 자체는 판별 가능.
