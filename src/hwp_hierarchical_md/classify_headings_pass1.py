...
def classify_and_merge(
    ...
    — Ollama가 없는 환경(예: GPU 없는 서비스 서버)에서도 같은 분류 로직을 쓸 수 있게 하려고
    2026-08-23에 추가한 확장점(자세한 배경은 `llm_backend.py` 참고).
    ...
    (README "LLM 백엔드 검증 메모" 참고) 호출부(`cli.py`의 `--max-candidates-per-call`,
    `api.py`의 `/v1/convert` 같은 이름 필드)에서 이 값을 명시적으로 낮출 수 있다 — 다만 이제는
    그 값이 "시작 크기"일 뿐, 실제로 그 제공자의 API가 토큰 한도 초과를 알려오면(2026-08-27
    추가) 아래에서 자동으로 배치를 줄여 재시도한다. 제공자마다 한도를 미리 추측해 하드코딩하는
    대신, 매번 그 제공자가 실제로 응답에 실어 보내는 정확한 수치(`RateLimitExceeded.limit_tokens`/
    `.requested_tokens`, `llm_backend.py` 참고)를 그대로 쓴다."""
    if backend is None:
        if model is None:
