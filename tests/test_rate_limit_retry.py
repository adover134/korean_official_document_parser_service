...
from hwp_hierarchical_md.classify_headings_pass1 import classify_and_merge
from hwp_hierarchical_md.llm_backend import RateLimitExceeded, _parse_rate_limit_error
...
def test_size_limit_error_shrinks_using_reported_limit_and_requested():
    ...
    with mock.patch("hwp_hierarchical_md.classify_headings_pass1.classify", fake_classify):
...
def test_rolling_window_exhausted_retries_same_size_after_wait(monkeypatch):
    ...
    sleeps: list[float] = []
    monkeypatch.setattr("hwp_hierarchical_md.classify_headings_pass1.time.sleep", sleeps.append)
...
def test_persistent_rate_limit_eventually_raises(monkeypatch):
    ...
    monkeypatch.setattr("hwp_hierarchical_md.classify_headings_pass1.time.sleep", lambda *_: None)

    def always_fails(cands, backend):
        raise RateLimitExceeded("계속 실패", is_size_limit=False, retry_after=0.0)

    with mock.patch("hwp_hierarchical_md.classify_headings_pass1.classify", always_fails):
        try:
